...
export class CommandPalette {
	...
	private getSourceLabel(source: PaletteCommand["source"]): string {
		switch (source) {
			case "builtin":
				return t("commandPalette.sources.builtin");
			case "extension":
				return t("commandPalette.sources.extension");
			case "prompt":
				return t("commandPalette.sources.prompt");
			case "skill":
				return t("commandPalette.sources.skill");
			case "runtime":
				return t("commandPalette.sources.runtime");
			default:
				return t("commandPalette.sources.other");
		}
	}
				...
				<div class="command-palette-card">
					...
					<div class="command-palette-list">
						${this.filteredCommands.length === 0
							? html`<div class="overlay-empty">${t("commandPalette.noMatch")}</div>`
							: this.filteredCommands.map(
									(command, index) => html`
										...
										>
											...
											<div class="command-row-source">${this.getSourceLabel(command.source)}</div>
										</button>
									`,
							  )}
					</div>
					...
					<div class="command-palette-footer">
						<span><kbd>↑</kbd><kbd>↓</kbd> ${t("commandPalette.hints.navigate")}</span>
						<span><kbd>Enter</kbd> ${t("commandPalette.hints.run")}</span>
						<span><kbd>Esc</kbd> ${t("commandPalette.hints.close")}</span>
					</div>
				</div>
			</div>
		`;
