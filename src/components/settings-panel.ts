...
export class SettingsPanel {
	...
	private desktopInstalling = false;
	...
	private readonly followUpModeSelect = new SettingsSelectDropdown({
		requestRender: () => {
			if (this.isOpen) this.render();
		},
	});
	private readonly themeProfileSelects: Record<ThemeVariant, SettingsSelectDropdown> = {
		light: new SettingsSelectDropdown({
			requestRender: () => {
				if (this.isOpen) this.render();
			},
		}),
		dark: new SettingsSelectDropdown({
			requestRender: () => {
				if (this.isOpen) this.render();
			},
		}),
	...
	constructor(container: HTMLElement) {
		...
		this.channelsSettings = new ChannelsSettings({
			requestRender: () => {
				if (this.isOpen) this.render();
			},
		});
	...
	setOnChannelsConfigChanged(callback: (() => void) | null): void {
		this.channelsSettings.setOnConfigChanged(callback);
	}

	setOnChannelsOAuthTerminal(callback: (() => void) | null): void {
		this.channelsSettings.setOnRequestOAuthTerminal(callback);
	}

	setOnExtensionsConfigChanged(callback: (() => void) | null): void {
		this.extensionsSettings.setOnConfigChanged(callback);
	}

	setOnExtensionsOpenPackages(callback: (() => void) | null): void {
		this.extensionsSettings.setOnOpenPackages(callback);
	}

	/** 定位扩展页的子视图（skills / extensions / mcp），供外部直达入口使用。 */
	setExtensionsView(view: ExtensionsViewId): void {
		this.extensionsSettings.setActiveView(view);
	}
	...
	setRuntimeProjectPath(projectPath: string | null): void {
		...
		this.extensionsSettings.setProjectPath(this.runtimeProjectPath);
	...
	private resolveDefaultThemeId(theme: ThemeVariant): string {
		...
		const preferred = `pi-desktop-default-${theme}`;
	...
	private baseThemePreview(theme: ThemeVariant): { accent: string; background: string; foreground: string } {
		...
		if (theme === "light") return { accent: "#0D0D0D", background: "#FFFFFF", foreground: "#0D0D0D" };
		return { accent: "#F5F5F5", background: "#181818", foreground: "#F5F5F5" };
	}
	...
	private async saveCreateThemeFromDialog(): Promise<void> {
		...
		if (!normalizedName) {
			this.createThemeDialogError = t("settings.createDialog.nameRequired");
			this.render();
			return;
		}
	...
	private async persistThemeFromProfile(theme: ThemeVariant, normalizedName: string): Promise<void> {
		...
		this.themeCatalogMessage = t("settings.createDialog.created", { name: fileStem });
	}
	...
	private async choosePiBinaryPathFromDialog(): Promise<void> {
		try {
			...
			const selected = await open({
				multiple: false,
				directory: false,
				title: t("settings.updates.selectBinary"),
			});
			...
			this.piPathActionMessage = err instanceof Error ? err.message : t("settings.updates.filePickerFailed");
			this.render();
		}
	}
	...
	private async savePiBinaryPathOverride(): Promise<void> {
		...
		try {
			...
			this.piPathActionMessage = normalized
				? t("settings.updates.overrideSaved")
				: t("settings.updates.overrideCleared");
			await this.refreshCliStatus();
			if (this.isRuntimeControlsEnabled()) {
				await this.refreshCompatibilityStatus();
			}
		} catch (err) {
			this.piPathActionMessage = err instanceof Error ? err.message : t("settings.updates.overrideSaveFailed");
		}
		this.render();
	}
	...
	/** 打开 Releases 页面手动下载（已停用应用内一键下载安装）。 */
	private async openDesktopReleasePage(): Promise<void> {
		if (this.desktopInstalling) return;
		if (!this.desktopStatus?.updateAvailable) return;
		this.desktopInstalling = true;
		this.desktopActionMessage = t("settings.updates.openingRelease");
		this.render();
		try {
			await openDesktopUpdate(this.desktopStatus);
			this.desktopActionMessage = t("settings.updates.openedRelease");
		} catch (err) {
			this.desktopActionMessage = err instanceof Error ? err.message : t("settings.updates.openFailed");
		} finally {
			this.desktopInstalling = false;
			this.render();
		}
	}
	...
	private async updateCliNow(): Promise<void> {
		...
		this.cliActionMessage = t("settings.updates.updatingViaNpm");
		this.render();
		try {
			const report = await rpcBridge.updateCliAndReport();
			if (report.success) {
				this.cliActionMessage = report.new_version
					? t("settings.updates.updateSuccessTo", { version: report.new_version })
					: t("settings.updates.updateSuccess");
			} else {
				// 把 npm 的输出尾段带出来，失败原因可见，不再只有「更新失败」
				const reason = report.error ?? report.output_tail;
				this.cliActionMessage = reason
					? `${t("settings.updates.updateFailed", { code: report.exit_code })}\n${reason}`
					: t("settings.updates.updateFailed", { code: report.exit_code });
			}
			await this.refreshCliStatus();
			if (this.isRuntimeControlsEnabled()) {
			...
			this.cliActionMessage = err instanceof Error ? err.message : t("settings.updates.updateFailedGeneric");
		} finally {
	...
	private async saveSettings(): Promise<void> {
		...
		try {
			const { invoke } = await import("@tauri-apps/api/core");
			// Merge over persisted settings so keys this panel does not manage
			// (e.g. max_running_runtimes, runtime_idle_timeout_minutes) survive a save.
			const current = (await invoke("load_settings").catch(() => null)) as Record<string, unknown> | null;
	...
	private renderToggle(
		...
		return html`
			<div class="settings-row">
				...
				<button class="toggle ${checked ? "on" : "off"}" ?disabled=${disabled} @click=${() => onChange(!checked)}>
					<span></span>
				</button>
			</div>
		`;
	...
	private getThemeProfileTitle(theme: ThemeVariant): string {
		switch (theme) {
			case "light":
				return t("settings.profile.lightTitle");
			case "dark":
				return t("settings.profile.darkTitle");
		}
	}

	private getThemeVariantLabel(theme: ThemeVariant): string {
		switch (theme) {
			case "light":
	...
	private renderThemeProfileCard(theme: ThemeVariant): TemplateResult {
		const title = this.getThemeProfileTitle(theme);
		const profile = this.getProfile(theme);
		const preview = this.profilePreview(theme);
			...
			<section class="appearance-profile-card">
				<div class="appearance-profile-header">
					...
					<button
						...
						title=${canCreateTheme ? t("settings.profile.createEnabledHint") : t("settings.profile.createDisabledHint")}
						@click=${() => this.createThemeFromProfile(theme)}
					>
						${t("settings.profile.createTheme")}
					</button>
					<div class="appearance-theme-select-wrap">
						${this.themeProfileSelects[theme].render({
							ariaLabel: title,
							value: selectedValue,
							triggerClass: "settings-select-trigger-lg",
							options: scopedOptions.map((entry) => ({ value: entry.id, label: entry.label })),
							onSelect: (value) => this.setProfileThemeName(theme, value),
						})}
					</div>
				</div>
				<div class="appearance-profile-row"><span>${t("settings.profile.accent")}</span>${this.renderColorControl(theme, "accent", preview.accent)}</div>
				<div class="appearance-profile-row"><span>${t("settings.profile.background")}</span>${this.renderColorControl(theme, "background", preview.background)}</div>
				<div class="appearance-profile-row"><span>${t("settings.profile.foreground")}</span>${this.renderColorControl(theme, "foreground", preview.foreground)}</div>
				<div class="appearance-profile-row">
					<span>${t("settings.profile.uiFont")}</span>
					<input
				...
				<div class="appearance-profile-row">
					<span>${t("settings.profile.codeFont")}</span>
					<input
						class="appearance-font-input"
				...
				<div class="appearance-profile-row">
					<span>${t("settings.profile.translucentSidebar")}</span>
				...
				<div class="appearance-profile-row">
					<span>${t("settings.profile.contrast")}</span>
	...
	private renderCreateThemeDialog(): TemplateResult | typeof nothing {
		...
		const variantLabel = this.getThemeVariantLabel(this.createThemeDialogTheme);
		return html`
			<div class="settings-subdialog-backdrop" @click=${() => this.closeCreateThemeDialog()}>
				<div class="settings-subdialog-card" @click=${(e: Event) => e.stopPropagation()}>
					<div class="settings-subdialog-title">${t("settings.createDialog.title", { variant: variantLabel })}</div>
					<div class="settings-subdialog-desc">${t("settings.createDialog.desc")}</div>
					<input
					...
					<div class="settings-subdialog-actions">
						<button class="ghost-btn" ?disabled=${this.createThemeDialogSaving} @click=${() => this.closeCreateThemeDialog()}>${t("common.cancel")}</button>
						<button class="ghost-btn" ?disabled=${this.createThemeDialogSaving} @click=${() => void this.saveCreateThemeFromDialog()}>
							${this.createThemeDialogSaving ? t("settings.saving") : t("settings.createDialog.saveTheme")}
						</button>
					</div>
				</div>
			</div>
				...
				<div class="settings-view-root">
					<div class="settings-view-header">
						<div class="settings-view-title-wrap">
							<div class="settings-view-title">${t("settings.nav.appearance.label")}</div>
						</div>
					</div>
						...
						<div class="settings-view-grid">
							<section class="settings-group settings-group-full">
								<div class="settings-section">
									<div class="settings-section-title">${t("settings.appearance.sectionTitle")}</div>
									<div class="settings-label">${t("settings.theme.label")}</div>
									<div class="settings-desc">${t("settings.shell.appearanceDesc")}</div>
									<div class="theme-grid" style="margin-top:10px;">
										<button class="theme-btn ${this.state.theme === "light" ? "active" : ""}" @click=${() => this.setTheme("light")}>${t("settings.theme.light")}</button>
										<button class="theme-btn ${this.state.theme === "dark" ? "active" : ""}" @click=${() => this.setTheme("dark")}>${t("settings.theme.dark")}</button>
										<button class="theme-btn ${this.state.theme === "system" ? "active" : ""}" @click=${() => this.setTheme("system")}>${t("settings.theme.system")}</button>
									</div>
								</div>
							</section>
							<section class="settings-group settings-group-full">
								<div class="settings-section">
									<div class="settings-section-title">${t("settings.shell.runtimeTitle")}</div>
									...
									${showAddProject
										? html`
											<div class="settings-actions" style="margin-top:10px;">
												<button class="ghost-btn" @click=${() => this.onRequestAddProject?.()}>${t("settings.addProject")}</button>
											</div>
										`
										: nothing}
								</div>
	...
	private getSettingsNavItems(): SettingsSectionNavItem[] {
		return [
			...
			{
				id: "appearance",
				label: t("settings.nav.appearance.label"),
				description: t("settings.nav.appearance.description"),
			},
			{
				id: "channels",
				label: t("settings.nav.channels.label"),
				description: t("settings.nav.channels.description"),
			},
			{
				id: "extensions",
				label: t("settings.nav.extensions.label"),
				...
				<section class="settings-group settings-group-full">
					<div class="settings-section">
						...
						<div class="appearance-theme-block">
							...
							${this.themeCatalogLoading ? html`<div class="ui-loading compact" role="status" aria-label=${t("settings.theme.loadingCatalog")}><span class="ui-loading-spinner"></span></div>` : nothing}
							${this.themeCatalogError ? html`<div class="settings-desc">${t("settings.theme.catalogError")} ${this.themeCatalogError}</div>` : nothing}
		...
		return html`
			<div class="settings-view-grid">
				<section class="settings-group">
					<div class="settings-section">
						<div class="settings-section-title">${t("settings.general.assistant")}</div>
						${!runtimeControlsEnabled ? html`<div class="settings-desc">${t("settings.general.runtimeNeededForToggles")}</div>` : nothing}
						${this.renderToggle(
							t("settings.general.autoCompaction"),
							t("settings.general.autoCompactionDesc"),
							this.state.autoCompactionEnabled,
							(v) => this.setAutoCompaction(v),
							!runtimeControlsEnabled,
						)}
						${this.renderToggle(
							t("settings.general.autoRetry"),
							t("settings.general.autoRetryDesc"),
				...
				<section class="settings-group">
					<div class="settings-section">
						<div class="settings-section-title">${t("settings.general.messageQueue")}</div>
						<div class="settings-row">
							<div>
								<div class="settings-label">${t("settings.general.steering")}</div>
								<div class="settings-desc">${t("settings.general.steeringDesc")}</div>
							</div>
							${this.steeringModeSelect.render({
								ariaLabel: t("settings.general.steering"),
								disabled: !runtimeControlsEnabled,
								value: this.state.steeringMode,
								options: [
									{ value: "one-at-a-time", label: t("settings.general.oneAtATime") },
									{ value: "all", label: t("settings.general.allQueued") },
								],
								onSelect: (value) => void this.setSteeringMode(value as QueueMode),
							})}
						</div>
						<div class="settings-row">
							<div>
								<div class="settings-label">${t("settings.general.followUp")}</div>
								<div class="settings-desc">${t("settings.general.followUpDesc")}</div>
							</div>
							${this.followUpModeSelect.render({
								ariaLabel: t("settings.general.followUp"),
								disabled: !runtimeControlsEnabled,
								...
								onSelect: (value) => void this.setFollowUpMode(value as QueueMode),
							})}
						</div>
					</div>
				</section>
	...
	private renderUpdatesSection(runtimeControlsEnabled: boolean, compatibilityChecks: string[]): TemplateResult {
		return html`
			<div class="settings-view-grid">
				<section class="settings-group settings-group-full">
					<div class="settings-section">
						<div class="settings-section-title">${t("settings.updates.cliCardTitle")}</div>
						${this.cliLoading
							? html`<div class="settings-desc">${t("settings.updates.checkingCli")}</div>`
							: html`
								<div class="settings-desc">${t("settings.updates.current")} <code>${this.cliStatus?.current_version || t("settings.updates.unknown")}</code> · ${t("settings.updates.latest")} <code>${this.cliStatus?.latest_version || t("settings.updates.unknown")}</code></div>
								${this.cliStatus
									? this.cliStatus.update_available
										? html`<div class="settings-desc">${t("settings.updates.newerCli")}</div>`
										: html`<div class="settings-desc">${t("settings.updates.noCliUpdate")}</div>`
									: html`<div class="settings-desc">${t("settings.updates.cliUnavailable")}</div>`}
								${this.cliStatus?.note ? html`<div class="settings-desc">${this.cliStatus.note}</div>` : null}
							`}
						<div class="settings-actions">
							<button class="ghost-btn" ?disabled=${this.cliLoading} @click=${() => this.refreshCliStatus()}>${t("settings.updates.refreshCli")}</button>
							<button
								class="ghost-btn"
								?disabled=${
									this.cliUpdating ||
							...
							>
								${this.cliUpdating ? t("settings.updates.updating") : t("settings.updates.updateNow")}
							</button>
						</div>
						${this.cliActionMessage ? html`<div class="settings-desc" style="white-space: pre-wrap; word-break: break-all;">${this.cliActionMessage}</div>` : null}
						<details class="settings-advanced">
							<summary>${t("settings.updates.advanced")}</summary>
							<div class="settings-row settings-row-top">
								<div>
									<div class="settings-label">${t("settings.updates.binaryOverride")}</div>
									<div class="settings-desc">${t("settings.updates.binaryDescPrefix")}<code>pi</code>${t("settings.updates.binaryDescSuffix")}</div>
									<div class="settings-desc">${t("settings.updates.examples")} <code>~/.npm-global/bin/pi</code>, <code>/usr/local/bin/pi</code>, <code>C:\\Users\\you\\AppData\\Roaming\\npm\\pi.cmd</code></div>
								</div>
							</div>
							<input
								type="text"
							...
							<div class="settings-actions" style="margin-top:8px;">
								<button class="ghost-btn" @click=${() => this.choosePiBinaryPathFromDialog()}>${t("settings.updates.browse")}</button>
								<button class="ghost-btn" ?disabled=${this.saving} @click=${() => this.savePiBinaryPathOverride()}>
									${this.saving ? t("settings.saving") : t("settings.updates.saveOverride")}
								</button>
								<button class="ghost-btn" ?disabled=${this.saving || this.state.piBinaryPath.trim().length === 0} @click=${() => this.clearPiBinaryPathOverride()}>
									${t("settings.updates.clearOverride")}
								</button>
							</div>
							${this.piPathActionMessage ? html`<div class="settings-desc">${this.piPathActionMessage}</div>` : null}
							${this.cliStatus?.update_command ? html`<div class="settings-desc" style="margin-top:8px;">${t("settings.updates.manualUpdate")} <code>${this.cliStatus.update_command}</code></div>` : null}
						</details>
					</div>

					<div class="settings-section">
						<div class="settings-section-title">${t("settings.updates.desktopCardTitle")}</div>
						${this.desktopLoading
							? html`<div class="settings-desc">${t("settings.updates.checkingDesktop")}</div>`
							: html`
								<div class="settings-desc">${t("settings.updates.current")} <code>${this.desktopStatus?.currentVersion || t("settings.updates.unknown")}</code> · ${t("settings.updates.latest")} <code>${this.desktopStatus?.latestVersion || t("settings.updates.unknown")}</code></div>
								${this.desktopStatus
									? this.desktopStatus.updateAvailable
										? html`<div class="settings-desc">${t("settings.updates.newerDesktop")}</div>`
										: html`<div class="settings-desc">${t("settings.updates.noDesktopUpdate")}</div>`
									: html`<div class="settings-desc">${t("settings.updates.desktopUnavailable")}</div>`}
								${this.desktopStatus?.assetName ? html`<div class="settings-desc">${t("settings.updates.recommendedInstaller")} <code>${this.desktopStatus.assetName}</code></div>` : null}
								${this.desktopStatus?.note ? html`<div class="settings-desc">${this.desktopStatus.note}</div>` : null}
							`}
						<div class="settings-actions">
							<button class="ghost-btn" ?disabled=${this.desktopLoading} @click=${() => this.refreshDesktopStatus()}>${t("settings.updates.refreshDesktop")}</button>
							<button class="ghost-btn" ?disabled=${this.desktopInstalling || !this.desktopStatus?.updateAvailable} @click=${() => this.openDesktopReleasePage()}>
								${this.desktopInstalling ? t("settings.updates.openingRelease") : t("settings.updates.goToReleases")}
							</button>
						</div>
						${this.desktopActionMessage ? html`<div class="settings-desc" style="white-space: pre-wrap; word-break: break-all;">${this.desktopActionMessage}</div>` : null}
					</div>

					${runtimeControlsEnabled
						? html`
							<details class="settings-advanced">
								<summary>${t("settings.updates.advancedDiagnostics")}</summary>
								<div class="settings-desc">${t("settings.updates.discovery")} <code>${this.cliStatus?.discovery || rpcBridge.discoveryInfo || t("settings.updates.unknown")}</code></div>
								<div class="settings-actions">
									<button class="ghost-btn" ?disabled=${this.compatibilityLoading} @click=${() => this.refreshCompatibilityStatus()}>
										${this.compatibilityLoading ? t("settings.updates.checkingRpc") : t("settings.updates.runRpcCheck")}
									</button>
								</div>
								${this.compatibilityReport
									? html`
										<div class="settings-desc">
											${t("settings.updates.rpcCompatibility")} ${this.compatibilityReport.ok ? t("settings.updates.rpcOk") : t("settings.updates.rpcFailed")}
						...
						`
						: html`<div class="settings-desc">${t("settings.updates.openProjectForDiagnostics")}</div>`}
				</section>
			</div>
		`;
	}
	...
	private renderActiveSectionSafe(
		...
		try {
			return this.renderActiveSection(section, runtimeControlsEnabled, hasProjectContext, compatibilityChecks);
		} catch (err) {
			...
			return html`
				<div class="settings-view-grid">
					<section class="settings-group settings-group-full">
						<div class="settings-section">
							<div class="settings-section-title">${t("settings.errors.sectionTitle")}</div>
							<div class="settings-desc">${t("settings.errors.sectionRender")}</div>
		...
		try {
			const template = html`
				<div class="settings-view-root">
					<div class="settings-view-header">
						<div class="settings-view-title-wrap">
							<div class="settings-view-title">${activeItem?.label ?? t("settings.title")}</div>
						</div>
					</div>
			...
			try {
				this.renderBasicSettingsShell(
					t("settings.errors.fallbackMessage"),
					{ showAddProject: false, warning: t("settings.errors.fallbackWarning") },
				);
			} catch {
				render(
					html`<div class="settings-view-root"><div class="settings-view-body"><div class="settings-desc">${t("settings.errors.unable")}</div></div></div>`,
					this.container,
				);
			}
		}
