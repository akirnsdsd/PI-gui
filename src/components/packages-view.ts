...
const DESKTOP_THEMES_DOC_URL = "https://github.com/akirnsdsd/PI-gui/blob/main/docs/THEMES_DESKTOP_MAPPING.md";
...
function parseSkillDoc(content: string): ParsedSkillDoc {
	...
	let title = attributes.name ? toTitleFromSlug(attributes.name) : t("packages.skillDoc.fallbackTitle");
	...
	for (const raw of body.split(/\r?\n/)) {
		...
		if (headingMatch) {
			...
			if (!title || title === t("packages.skillDoc.fallbackTitle")) title = activeHeading;
			continue;
		}
	...
	if (!title) title = t("packages.skillDoc.fallbackTitle");
...
function sourceKindLabel(kind: "npm" | "git" | "url" | "local" | "unknown"): string {
	switch (kind) {
		...
		case "url":
			return "url";
		case "local":
			return t("packages.kinds.sourceLocal");
		default:
			return t("packages.kinds.sourceUnknown");
	}
}

function resourceKindLabel(kind: "prompt" | "skill"): string {
	switch (kind) {
		case "prompt":
			return t("packages.kinds.prompt");
		default:
			return t("packages.kinds.skill");
	}
}
...
export class PackagesView {
	...
	private resourcesRuntimeUnavailable = false;
	...
	/** 弹窗自定义下拉实例缓存：键 = 语义位置（同一弹窗可能同时渲染多个下拉）。 */
	private readonly settingsSelects = new Map<string, SettingsSelectDropdown>();
	...
	/** 取/建一个自定义下拉实例；requestRender 走整视图重渲染（与 settings-panel 同款）。 */
	private settingsSelect(key: string): SettingsSelectDropdown {
		let select = this.settingsSelects.get(key);
		if (!select) {
			select = new SettingsSelectDropdown({ requestRender: () => this.render() });
			this.settingsSelects.set(key, select);
		}
		return select;
	}
	...
	private validateResourceName(name: string, kind: "prompt" | "skill"): string | null {
		const trimmed = name.trim();
		if (!trimmed) return t("packages.editor.nameRequired");
		if (kind === "skill") {
			if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(trimmed)) {
				return t("packages.editor.skillNameChars");
			}
			if (trimmed.length > 64) return t("packages.editor.skillNameLength");
			return null;
		}
		if (!/^[a-z0-9][a-z0-9_-]*$/.test(trimmed)) {
			return t("packages.editor.promptNameChars");
		}
		if (trimmed.length > 64) return t("packages.editor.promptNameLength");
		return null;
	}
	...
	private createPromptTemplateContent(description: string): string {
		const safeDescription = description.trim() || "描述什么时候使用这个模板。";
	...
	private createSkillTemplateContent(name: string, description: string): string {
		...
		const safeDescription = description.trim() || "描述这个技能做什么、什么时候使用。";
	...
	private async migrateLegacyNotifyPackageIfNeeded(): Promise<boolean> {
		...
		if (!hasPreferred) {
			const installOk = await this.executePackageCommand(["install", SMART_NOTIFY_INSTALL_SOURCE], {
				...
				statusText: t("packages.status.installing", { source: SMART_NOTIFY_INSTALL_SOURCE }),
				refreshOnSuccess: false,
			});
			if (!installOk) {
				this.commandStatus = t("packages.status.installFailed", { source: SMART_NOTIFY_INSTALL_SOURCE });
				return false;
			}
			changed = true;
		}
		...
		const removeOk = await this.executePackageCommand(["remove", removeSource], {
			scope: "global",
			appendLocalFlag: false,
			statusText: t("packages.status.removing", { source: removeSource }),
			refreshOnSuccess: false,
		});
		if (!removeOk) {
			if (changed) {
				this.commandStatus = t("packages.status.installedButLegacyRemoveFailed", { source: SMART_NOTIFY_INSTALL_SOURCE, legacy: removeSource });
			}
			return changed;
		}

		this.commandStatus = hasPreferred
			? t("packages.status.removedLegacyNotify", { source: removeSource })
			: t("packages.status.migratedNotify", { source: SMART_NOTIFY_INSTALL_SOURCE });
		return true;
	}
	...
	private async scanPromptResources(scope: "global" | "project"): Promise<DiscoveredResourceItem[]> {
		...
		for (const entry of entries) {
			...
			out.push({
				...
				kind: "prompt",
				name,
				description: t("packages.meta.localPromptTemplate"),
				commandText: `/${name}`,
				path,
			...
			for (const entry of entries) {
				...
				if (lower === "skill.md") {
					...
					out.push({
						...
						kind: "skill",
						name: skillName,
						description: t("packages.meta.localSkill"),
	...
	private async refreshDiscoveredResources(): Promise<void> {
		...
		try {
			// 无可用 runtime（未打开项目/无运行中会话）时不发 RPC：
			// 「已安装」区块降级为空态提示，不甩错误横幅。
			const runtimeRunning = await rpcBridge.refreshRunningState().catch(() => rpcBridge.isConnected);
			this.resourcesRuntimeUnavailable = !runtimeRunning;
			if (!runtimeRunning) {
	...
	private async findExistingProjectAutoRenameConfig(projectPath: string): Promise<{ path: string; scopeLabel: string } | null> {
		...
		const projectLabel = option?.name || pathBaseName(normalizedProjectPath) || t("packages.autoRename.projectFallback");
		const candidates = [
		...
		for (const candidate of candidates) {
			try {
				if (await exists(candidate)) {
					return { path: candidate, scopeLabel: t("packages.autoRename.scopeProjectNamed", { name: projectLabel }) };
				}
			} catch {
				// ignore path errors
			}
	...
	private async resolveProjectAutoRenameConfigPath(projectPath: string): Promise<{ path: string; scopeLabel: string }> {
		...
		return {
			...
			scopeLabel: t("packages.autoRename.scopeProjectNamed", { name: projectLabel }),
		};
	}
	...
	private async resolveAutoRenameConfigPathPreference(options?: {
		...
		if (requestedScope === "global") {
			return { path: globalPath, scopeLabel: t("packages.autoRename.scopeGlobal") };
		}

		if (requestedScope === "project") {
			const projectPath = requestedProjectPath || this.autoRenameConfigTargetProjectPath || this.currentProjectPath || "";
			if (!projectPath) {
				throw new Error(t("packages.autoRename.noProjectSelected"));
			}
	...
	private async setAutoRenameSaveScope(source: string, scope: "global" | "project"): Promise<void> {
		...
		if (scope === "project") {
			...
			const selected = this.autoRenameProjectOptions.find((item) => item.id === this.autoRenameConfigTargetProjectId)
				...
				?? null;
			if (!selected) {
				this.autoRenameConfigError = t("packages.autoRename.noProjectAvailable");
				this.render();
				return;
			}
			this.autoRenameConfigTargetProjectId = selected.id;
	...
	private async saveAutoRenameConfig(source: string): Promise<void> {
		...
		this.autoRenameConfigError = "";
		this.packageConfigStatus = t("packages.autoRename.savingSettings");
		this.render();
		try {
			...
			if (targetScope === "project" && !targetProjectPath) {
				throw new Error(t("packages.autoRename.chooseProject"));
			}
			...
			if (!configPath) {
				throw new Error(t("packages.autoRename.resolvePathFailed"));
			}
			...
			if (primaryModelValue && !primaryModel) {
				throw new Error(t("packages.autoRename.primaryModelFormat"));
			}
			if (fallbackModelValue && !fallbackModel) {
				throw new Error(t("packages.autoRename.fallbackModelFormat"));
			}
			...
			this.packageConfigStatus = t("packages.autoRename.savedTo", {
				path: configPath,
				scope: scopeLabel ? `（${scopeLabel}）` : "",
			});
			this.commandStatus = t("packages.autoRename.saved");
		} catch (err) {
			const message = err instanceof Error ? err.message : String(err);
			this.autoRenameConfigError = message;
			this.packageConfigStatus = t("packages.autoRename.saveFailed", { message });
			this.commandStatus = t("packages.autoRename.settingsFailed", { message });
		} finally {
	...
	async openExtensionConfigBySource(source: string): Promise<boolean> {
		...
		const item: ExtensionSurfaceItem = {
			...
			description: t("packages.meta.installedExtension"),
	...
	private ensureSyntheticExtensionConfigCommand(
		...
		if (!currentCommands.some((command) => commandNamesMatch(command.name, normalizedCommand))) {
			const hintedDescription = withExtensionCommandUsageHint(
				normalizedCommand,
				t("packages.config.runConfigCommand"),
			);
	...
	private async loadPackageConfigFromPackage(source: string, commands: PackageConfigCommand[]): Promise<void> {
		...
		try {
			...
			if (loadedFromPath) {
				...
				this.packageConfigStatus = t("packages.config.loadedModelFrom", {
					model: loadedModelValue || t("packages.config.unknownModel"),
					path: loadedFromPath,
				});
				...
				this.packageConfigStatus = t("packages.config.noSavedModel");
			}
			this.render();
		} catch {
			// ignore errors
	...
	private async runPackageConfigCommand(command: PackageConfigCommand, args = ""): Promise<void> {
		...
		const actionVerb = isModelSave ? t("packages.config.verbSave") : t("packages.config.verbRun");
		this.packageConfigStatus = t("packages.config.statusRunning", { verb: actionVerb });
		this.render();
		try {
			await rpcBridge.prompt(promptText);
			this.packageConfigStatus = t("packages.config.statusDone", { verb: actionVerb });
			this.commandStatus = t("packages.config.statusDoneFor", {
				label: this.activePackageConfigLabel || t("packages.config.packageFallback"),
				verb: actionVerb,
			});
			...
			if (supportsModelPicker && this.activePackageConfigSource && parsedModelArg) {
				try {
					...
					if (writtenPaths.length > 0) {
						this.packageConfigStatus = t("packages.config.statusDonePersisted", { verb: actionVerb, path: writtenPaths.join(" · ") });
					} else if (writeErrors.length > 0) {
						this.packageConfigStatus = t("packages.config.statusDonePersistFailed", { verb: actionVerb, error: writeErrors.join("; ") });
					}
				} catch (err) {
					const message = err instanceof Error ? err.message : String(err);
					this.packageConfigStatus = t("packages.config.statusDonePersistFailed", { verb: actionVerb, error: message });
				}
			}
		} catch (err) {
			const message = err instanceof Error ? err.message : String(err);
			this.packageConfigStatus = t("packages.config.statusFailed", { verb: actionVerb, message });
			this.commandStatus = t("packages.config.commandFailed", { message });
		} finally {
	...
	private renderPackageConfigCommandEditor(source: string, command: PackageConfigCommand): TemplateResult {
		...
		const title = command.description?.trim() || t("packages.config.fallbackTitle");
		const defaultDescription = supportsModelPicker ? t("packages.config.chooseModelAndSave") : t("packages.config.applyArguments");
		const commandDescription = isSmartNotifyCommand && !supportsModelPicker
			? t("packages.config.smartNotifyHint")
			: defaultDescription;
		const argsPlaceholder = isSmartNotifyCommand
			? "status | reload | on | off | test idle|permission|question|error"
			: t("packages.config.argsPlaceholder");
		const buttonLabel = supportsModelPicker ? t("common.save") : t("packages.config.apply");
		const buttonBusyLabel = supportsModelPicker ? t("packages.config.saving") : t("packages.config.applying");
		...
		return html`
			<div class="packages-config-command-card">
				...
				${isSmartNotifyCommand && !supportsModelPicker
					? html`<div class="packages-section-submeta">${t("packages.config.examplesLabel")}</div>`
					: nothing}

				${supportsModelPicker
					? html`
						<div class="packages-config-field">
							<label class="packages-config-field-label">${t("packages.config.modelLabel")}</label>
							${this.settingsSelect(`config-model:${source}:${command.name}`).render({
								ariaLabel: t("packages.config.modelLabel"),
								...
								options: this.configModelsLoading
									? [{ value: "", label: t("packages.config.loadingModels") }]
									: [
										{ value: "", label: t("packages.config.usePackageDefault") },
										...(currentArgs && !selectedModel ? [{ value: currentArgs, label: currentArgs }] : []),
										...this.configModels.map((model) => ({ value: modelRef(model.provider, model.id), label: model.label })),
									],
								onSelect: (value) => {
									this.setPackageConfigCommandArg(source, command.name, value);
									this.render();
					...
					: html`
						<div class="packages-config-field">
							<label class="packages-config-field-label">${t("packages.config.argumentsLabel")}</label>
							<input
	...
	private renderAutoRenameConfigEditor(source: string, command: PackageConfigCommand): TemplateResult {
		...
		const usageHint = extensionCommandUsageHint(command.name) || t("packages.autoRename.usageHintFallback");
		const supportsProjectScope = Boolean(this.currentProjectPath);
		const configScopeLabel = this.autoRenameConfigScopeLabel || (supportsProjectScope ? t("packages.autoRename.scopeProjectPi") : t("packages.autoRename.scopeGlobal"));
		...
		const autoModeHelper = extensionDisabled
			? t("packages.autoRename.helperDisabled")
			: prefixOnlyActive
				? t("packages.autoRename.helperPrefixOnly")
				: hasExplicitModels
					? t("packages.autoRename.helperLocked")
					: this.autoRenameConfig.modelSelection === "current"
						? t("packages.autoRename.helperCurrent")
						: t("packages.autoRename.helperCheapest");

		const resolutionOrder = [
			hasPrimaryModel ? t("packages.autoRename.orderPrimary", { model: modelValue }) : null,
			hasFallbackModel ? t("packages.autoRename.orderFallback", { model: fallbackModelValue }) : null,
			this.autoRenameConfig.modelSelection === "cheapest" ? t("packages.autoRename.orderAutoCheapest") : t("packages.autoRename.orderAutoCurrent"),
		]
		...
		return html`
			<div class="packages-config-command-card">
				<div class="packages-config-command-name">${t("packages.autoRename.title")}</div>
				<div class="packages-config-command-desc">${usageHint}</div>
				<div class="packages-section-submeta">${t("packages.autoRename.resolutionOrder", { order: resolutionOrder })}</div>

				${this.autoRenameConfigLoading
					? html`<div class="ui-loading compact" role="status" aria-label=${t("packages.autoRename.loading")}><span class="ui-loading-spinner"></span></div>`
					: html`
						<div class="packages-config-field-grid">
							<div class="packages-config-toggle-row ${busy ? "disabled" : ""}">
								<span class="packages-config-field-label">${t("packages.autoRename.enabled")}</span>
								<label class="packages-config-toggle-switch" aria-label=${t("packages.autoRename.enabled")}>
									<input
										type="checkbox"
							...
							<div class="packages-config-toggle-row ${busy || extensionDisabled ? "disabled" : ""}">
								<span class="packages-config-field-label">${t("packages.autoRename.prefixOnly")}</span>
								<label class="packages-config-toggle-switch" aria-label=${t("packages.autoRename.prefixOnly")}>
									<input
										type="checkbox"
										class="packages-config-toggle-input"
						...
						<div class="packages-config-field-grid">
							<div class="packages-config-field">
								<label class="packages-config-field-label">${t("packages.autoRename.primaryModel")}</label>
								${this.settingsSelect(`auto-rename:${command.name}:model`).render({
									ariaLabel: t("packages.autoRename.primaryModel"),
									triggerClass: "settings-select-trigger-field",
									disabled: modelControlsDisabled || this.configModelsLoading,
									value: modelValue,
									options: [
										{ value: "", label: t("packages.autoRename.noneUseAuto") },
										...(hasPrimaryModel && !primaryInOptions
											? [{ value: modelValue, label: t("packages.autoRename.savedOption", { model: modelValue }) }]
											: []),
										...modelOptions.map((model) => ({ value: modelRef(model.provider, model.id), label: model.label })),
									],
									onSelect: (value) => this.setAutoRenameConfigField("model", value),
								})}
							</div>
							<div class="packages-config-field">
								<label class="packages-config-field-label">${t("packages.autoRename.fallbackModel")}</label>
								${this.settingsSelect(`auto-rename:${command.name}:fallback-model`).render({
									ariaLabel: t("packages.autoRename.fallbackModel"),
									triggerClass: "settings-select-trigger-field",
									disabled: modelControlsDisabled || this.configModelsLoading,
									value: fallbackModelValue,
									options: [
										{ value: "", label: t("packages.autoRename.none") },
										...(hasFallbackModel && !fallbackInOptions
											? [{ value: fallbackModelValue, label: t("packages.autoRename.savedOption", { model: fallbackModelValue }) }]
											: []),
										...modelOptions.map((model) => ({ value: modelRef(model.provider, model.id), label: model.label })),
									],
									onSelect: (value) => this.setAutoRenameConfigField("fallbackModel", value),
								})}
							</div>
						</div>
						...
						<div class="packages-config-field-grid">
							<div class="packages-config-field">
								<label class="packages-config-field-label">${t("packages.autoRename.autoMode")}</label>
								${this.settingsSelect(`auto-rename:${command.name}:model-selection`).render({
									ariaLabel: t("packages.autoRename.autoMode"),
									triggerClass: "settings-select-trigger-field",
									disabled: modeControlsDisabled,
									value: this.autoRenameConfig.modelSelection,
									options: [
										{ value: "current", label: t("packages.autoRename.preferCurrent") },
										{ value: "cheapest", label: t("packages.autoRename.preferCheapest") },
									],
									onSelect: (value) => this.setAutoRenameConfigField("modelSelection", value === "cheapest" ? "cheapest" : "current"),
								})}
							</div>
							<div class="packages-config-field">
								<label class="packages-config-field-label">${t("packages.autoRename.prefixLabel")}</label>
								<input
									class="packages-config-input"
									type="text"
									placeholder=${t("packages.autoRename.prefixPlaceholder")}
						...
						<details class="packages-config-advanced">
							<summary>${t("packages.autoRename.moreOptions")}</summary>
							<div class="packages-config-field-grid">
								<div class="packages-config-field">
									<label class="packages-config-field-label">${t("packages.autoRename.deterministicFallback")}</label>
									${this.settingsSelect(`auto-rename:${command.name}:fallback-deterministic`).render({
										ariaLabel: t("packages.autoRename.deterministicFallback"),
										triggerClass: "settings-select-trigger-field",
										disabled: busy || extensionDisabled || prefixOnlyActive,
										value: this.autoRenameConfig.fallbackDeterministic,
										options: [
											{ value: "readable-id", label: "readable-id" },
											{ value: "words", label: "words" },
											{ value: "truncate", label: "truncate" },
											{ value: "none", label: "none" },
										],
										onSelect: (value) =>
											this.setAutoRenameConfigField("fallbackDeterministic", value as AutoRenameConfigDraft["fallbackDeterministic"]),
									})}
								</div>
								<div class="packages-config-field">
									<label class="packages-config-field-label">${t("packages.autoRename.prefixCommand")}</label>
									<input
										class="packages-config-input"
										type="text"
										placeholder=${t("packages.autoRename.prefixCommandPlaceholder")}
							...
							<div class="packages-config-field-grid">
								<div class="packages-config-field">
									<label class="packages-config-field-label">${t("packages.autoRename.readableIdSuffix")}</label>
									${this.settingsSelect(`auto-rename:${command.name}:readable-id-suffix`).render({
										ariaLabel: t("packages.autoRename.readableIdSuffix"),
										triggerClass: "settings-select-trigger-field",
										disabled: basicControlsDisabled,
										value: boolSelect(this.autoRenameConfig.readableIdSuffix),
										options: [
											{ value: "false", label: t("packages.autoRename.off") },
											{ value: "true", label: t("packages.autoRename.on") },
										],
										onSelect: (value) => this.setAutoRenameConfigField("readableIdSuffix", value === "true"),
									})}
								</div>
								<div class="packages-config-field">
									<label class="packages-config-field-label">${t("packages.autoRename.debugNotifications")}</label>
									${this.settingsSelect(`auto-rename:${command.name}:debug`).render({
										ariaLabel: t("packages.autoRename.debugNotifications"),
										triggerClass: "settings-select-trigger-field",
										disabled: basicControlsDisabled,
										...
										onSelect: (value) => this.setAutoRenameConfigField("debug", value === "true"),
									})}
								</div>
							</div>
						</details>
				...
				<div class="packages-config-command-actions">
					<div class="packages-config-save-inline">
						<span class="packages-config-field-label">${t("packages.autoRename.saveTo")}</span>
						${this.settingsSelect(`auto-rename:${command.name}:save-scope`).render({
							ariaLabel: t("packages.autoRename.saveTo"),
							triggerClass: "settings-select-trigger-field",
							disabled: busy,
							value: saveScope,
							options: [
								{ value: "global", label: t("packages.autoRename.scopeGlobal") },
								{ value: "project", label: t("packages.autoRename.scopeProject"), disabled: !canUseProjectScope },
							],
							onSelect: (value) => {
								...
								void this.setAutoRenameSaveScope(source, scope);
							},
						})}
						${saveScope === "project"
							? this.settingsSelect(`auto-rename:${command.name}:target-project`).render({
								ariaLabel: t("packages.autoRename.selectProject"),
								triggerClass: "settings-select-trigger-field",
								disabled: busy || !canUseProjectScope,
								value: activeProjectTarget?.id || "",
								options: [
									{ value: "", label: t("packages.autoRename.selectProject") },
									...projectOptions.map((item) => ({ value: item.id, label: item.name })),
								],
								onSelect: (value) => void this.setAutoRenameTargetProject(source, value),
							})
							: nothing}
					</div>
					<button
					...
					>
						${this.autoRenameConfigSaving ? t("packages.autoRename.saving") : t("packages.autoRename.saveSettings")}
					</button>
					<button
						class="ghost-btn"
					...
					>
						${t("packages.autoRename.showConfig")}
					</button>
					<button
						class="ghost-btn"
					...
					>
						${t("packages.autoRename.runTest")}
					</button>
				</div>

				${this.autoRenameConfigPath
					? html`<div class="packages-section-submeta">${t("packages.autoRename.configFile", { path: this.autoRenameConfigPath, scope: configScopeLabel ? `（${configScopeLabel}）` : "" })}</div>`
					: nothing}
				${targetProjectMissing ? html`<div class="packages-config-inline-error">${t("packages.autoRename.chooseProject")}</div>` : nothing}
			...
			<div class="overlay" @click=${(event: Event) => event.target === event.currentTarget && this.closeActivePackageConfig()}>
				<div class="overlay-card packages-config-modal">
					<div class="overlay-header">
						<div>
							<div class="packages-config-modal-title">${t("packages.config.modalTitle", { label: this.activePackageConfigLabel })}</div>
							<div class="packages-config-modal-sub">${t("packages.config.modalSub")}</div>
						</div>
					...
					<div class="overlay-body packages-config-modal-body">
						...
						<div class="packages-config-modal-actions">
							<button class="ghost-btn" ?disabled=${this.loadingConfig || this.runningCommand} @click=${() => void this.refreshPackages(false)}>
								${t("packages.config.refreshCommands")}
							</button>
							${hasModelCommand
								? html`
									<button class="ghost-btn" ?disabled=${this.configModelsLoading || this.runningConfigCommand} @click=${() => void this.ensureConfigModelsLoaded(true)}>
										${this.configModelsLoading ? t("packages.config.refreshingModels") : t("packages.config.refreshModels")}
									</button>
								`
								: nothing}
						</div>

						${this.configModelsError ? html`<div class="packages-config-inline-error">${t("packages.config.modelLookupFailed", { message: this.configModelsError })}</div>` : nothing}
		...
		try {
			...
			for (const entry of entries) {
				...
				const description = typeof parsed.description === "string" && parsed.description.trim()
					...
					: variant === "dark" ? t("packages.kinds.themeDark") : t("packages.kinds.themeLight");
	...
	private async materializeRecommendedSkillFromPackage(item: RecommendedSkillSurfaceItem): Promise<string | null> {
		...
		if (!(await exists(targetSkillPath))) {
			throw new Error(`${t("packages.status.missingSkillMd", { name: item.definition.skillName })}${copyError ? ` ${copyError}` : ""}`);
		}
	...
	private async executePackageCommand(
		...
		try {
			...
			if (result.exit_code !== 0) {
				this.commandStatus = (stdErr || stdOut || t("packages.status.commandFailedExit", { code: result.exit_code })).trim();
				return false;
			}
			...
			this.commandStatus = t("packages.status.commandFailed", { message: err instanceof Error ? err.message : String(err) });
			return false;
		} finally {
	...
	private async installPackage(source: string, scope: "global" | "local"): Promise<void> {
		...
		const success = await this.executePackageCommand(["install", trimmed], {
			...
			appendLocalFlag: false,
			statusText: t("packages.status.installing", { source: trimmed }),
			refreshOnSuccess: true,
		});
		if (success) {
			this.commandStatus = t("packages.status.installed", { source: trimmed });
		}
	}
	...
	private async removePackage(source: string, scope: "global" | "local"): Promise<void> {
		...
		const success = await this.executePackageCommand(["remove", trimmed], {
			scope: effectiveScope,
			appendLocalFlag: false,
			statusText: t("packages.status.removing", { source: trimmed }),
			refreshOnSuccess: true,
		});
		if (success) {
			const removedProviders = await this.cleanupProviderAuthForRemovedPackage(trimmed);
			if (removedProviders.length > 0) {
				this.commandStatus = t("packages.status.removedWithAuthCleanup", { source: trimmed, providers: removedProviders.join(", ") });
			} else {
				this.commandStatus = t("packages.status.removed", { source: trimmed });
			}
		}
	}
	...
	private async updatePackages(): Promise<void> {
		...
		const success = await this.executePackageCommand(["update"], {
			scope: effectiveScope,
			appendLocalFlag: false,
			statusText: t("packages.status.updating"),
			refreshOnSuccess: true,
		});
		if (success) {
			this.commandStatus = t("packages.status.updated");
		}
	}
	...
	private async listPackages(): Promise<void> {
		...
		const success = await this.executePackageCommand(["list"], {
			scope: effectiveScope,
			appendLocalFlag: false,
			statusText: t("packages.status.refreshingList"),
			refreshOnSuccess: true,
		});
		if (success) {
			this.commandStatus = t("packages.status.refreshedList");
		}
	}
	...
	private async runResourceCommand(item: DiscoveredResourceItem): Promise<void> {
		if (this.runningCommand || this.runningConfigCommand) return;
		this.runningCommand = true;
		this.commandStatus = t("packages.status.applyingTemplate", { command: item.commandText });
		this.commandOutput = `${this.commandOutput ? `${this.commandOutput}\n\n` : ""}[resource] ${item.commandText}\n`;
		this.render();
		try {
			await rpcBridge.prompt(item.commandText);
			this.commandStatus = t("packages.status.appliedTemplate", { command: item.commandText });
		} catch (err) {
			this.commandStatus = t("packages.status.runFailed", { command: item.commandText, message: err instanceof Error ? err.message : String(err) });
		} finally {
			this.runningCommand = false;
			this.render();
		}
	...
	private async applyPromptTemplate(item: DiscoveredResourceItem): Promise<void> {
		await this.runResourceCommand(item);
	}
	...
	private async stageResourceCommandInChat(commandText: string, successLabel: string): Promise<void> {
		if (!this.onInsertPromptTemplate) {
			this.commandStatus = t("packages.status.chatComposerUnavailable");
			this.render();
			return;
		}
		try {
			...
			this.commandStatus = t("packages.status.prepareInChatFailed", { message: err instanceof Error ? err.message : String(err) });
		}
		this.render();
	}

	private async runSkill(item: DiscoveredResourceItem): Promise<void> {
		await this.stageResourceCommandInChat(item.commandText, t("packages.status.preparedInChat", { command: item.commandText }));
	}
	...
	private async installRecommendedSkill(item: RecommendedSkillSurfaceItem): Promise<void> {
		...
		if (item.definition.sourceKind !== "local" && !packageInstalled) {
			this.commandStatus = t("packages.status.installSkillFailed", { name: item.definition.name });
			this.render();
			return;
		}
		...
		try {
			...
			const message = err instanceof Error ? err.message : String(err);
			this.commandStatus = t("packages.status.skillFilesPrepareFailed", { name: item.definition.name, message });
			this.render();
		}
		...
		if (!resolvedSkill) {
			this.commandStatus = t("packages.status.skillNotDiscovered", { name: item.definition.name, skill: item.definition.skillName });
			this.render();
			return;
		}
		const commandText = resolvedSkill.commandText;
		const setupSuffix = item.definition.setupHint ? t("packages.status.setupSuffix", { hint: item.definition.setupHint }) : "";
		const skillPath = materializedSkillPath || resolvedSkill.path;
		const locationSuffix = skillPath ? t("packages.status.skillFilesAt", { path: pathDirName(skillPath) }) : "";
		await this.stageResourceCommandInChat(commandText, `${t("packages.status.preparedInChat", { command: commandText })}${locationSuffix}${setupSuffix}`);
	...
	private insertPromptTemplate(item: DiscoveredResourceItem): void {
		void this.stageResourceCommandInChat(item.commandText, t("packages.status.insertedIntoComposer", { command: item.commandText }));
	}
	...
	private async triggerCreatorSkillInChat(): Promise<void> {
		...
		await this.stageResourceCommandInChat(commandText, t("packages.status.preparedInChat", { command: commandText }));
	}
...
Execute the required file creation/edits directly, then summarize exactly which files were created.`;
	...
	private async runResourceCreator(): Promise<void> {
		...
		if (!this.resourceCreatorBrief.trim()) {
			this.resourceCreatorError = t("packages.creator.describeFirst");
			this.render();
			return;
		}
		this.resourceCreatorScope = "global";
		...
		this.resourceCreatorError = "";
		this.resourceStatus = t("packages.creator.preparingDraft");
		this.render();
		...
		try {
			...
			const statusText = loadedCommand
				? t("packages.creator.preparedReady")
				: t("packages.creator.preparedPending");
		...
		return html`
			<div class="overlay" @click=${(event: Event) => event.target === event.currentTarget && this.closeResourceCreatorModal()}>
				<div class="overlay-card packages-config-modal packages-item-modal">
					<div class="overlay-header">
						<div>
							<div class="packages-config-modal-title">${creatingSkill ? t("packages.creator.titleSkill") : t("packages.creator.titleResource")}</div>
							<div class="packages-config-modal-sub">${t("packages.creator.sub")}</div>
						</div>
					...
					<div class="overlay-body packages-config-modal-body">
						<div class="packages-config-field-grid">
							${!this.resourceCreatorKindLocked
								? html`
									<div class="packages-config-field">
										<label class="packages-config-field-label">${t("packages.creator.typeLabel")}</label>
										${this.settingsSelect("resource-creator:kind").render({
											ariaLabel: t("packages.creator.typeLabel"),
											triggerClass: "settings-select-trigger-field",
											disabled: this.resourceCreatorRunning,
											value: this.resourceCreatorKind,
											options: [
												{ value: "auto", label: t("packages.creator.typeAuto") },
												{ value: "prompt", label: t("packages.creator.typePrompt") },
												{ value: "skill", label: t("packages.creator.typeSkill") },
											],
											onSelect: (value) => {
												this.resourceCreatorKind = value as "auto" | "prompt" | "skill";
												this.render();
											},
										})}
									</div>
							...
							<div class="packages-config-field">
								<label class="packages-config-field-label">${t("packages.creator.destination")}</label>
								<div class="packages-section-submeta">${globalTarget}</div>
								<div class="packages-section-submeta">${t("packages.creator.destinationHint")}</div>
							</div>
						</div>

						<div class="packages-config-field">
							<label class="packages-config-field-label">${t("packages.creator.briefLabel")}</label>
							<textarea
								class="packages-config-textarea"
								placeholder=${t("packages.creator.briefPlaceholder")}
								?disabled=${this.resourceCreatorRunning}
						...
						<div class="packages-config-modal-actions">
							<button class="ghost-btn" ?disabled=${this.resourceCreatorRunning} @click=${() => this.closeResourceCreatorModal()}>${t("common.cancel")}</button>
							<button class="ghost-btn" ?disabled=${this.resourceCreatorRunning} @click=${() => void this.runResourceCreator()}>
								${this.resourceCreatorRunning ? t("packages.creator.preparing") : t("packages.creator.prepareInChat")}
							</button>
						</div>
						<div class="packages-section-submeta">${t("packages.creator.footerPrefix")}<code>/skill:${RESOURCE_CREATOR_SKILL_NAME}</code>${t("packages.creator.footerSuffix")}</div>
	...
	private openCreateResourceEditor(kind: "prompt" | "skill"): void {
		...
		this.resourceEditorDescription = kind === "skill"
			? t("packages.editor.defaultSkillDescription")
			: t("packages.editor.defaultPromptDescription");
	...
	private async saveResourceEditor(): Promise<void> {
		...
		if (!this.resourceEditorContent.trim()) {
			this.resourceEditorError = t("packages.editor.contentRequired");
			this.render();
			return;
		}
		...
		this.resourceEditorError = "";
		this.resourceStatus = this.resourceEditorMode === "create" ? t("packages.editor.savingNew") : t("packages.editor.savingExisting");
		this.render();
		...
		try {
			...
			if (!targetPath) {
				throw new Error(t("packages.editor.resolvePathFailed"));
			}

			const root = this.resourceRootPath(kind, this.resourceEditorScope);
			if (!root) throw new Error(t("packages.editor.resolveRootFailed"));
	...
	private async deleteResource(item: DiscoveredResourceItem): Promise<void> {
		...
		// 技能的 path 指向 SKILL.md，删除目标是整个技能目录。
		const targetPath =
			item.kind === "skill" && /[\\/]skill\.md$/i.test(item.path)
				? item.path.replace(/[\\/]skill\.md$/i, "")
				: item.path;
		const confirmed = await confirmDialog({
			title: t("packages.editor.deleteTitle", { kind: resourceKindLabel(item.kind) }),
			desc: t("packages.editor.deleteConfirmTrash", { kind: resourceKindLabel(item.kind), name: item.name, path: targetPath }),
			confirmLabel: t("common.delete"),
			danger: true,
		});
		if (!confirmed) return;
		this.deletingResource = true;
		this.resourceStatus = t("packages.editor.trashing", { name: item.name });
		this.resourceEditorError = "";
		this.render();
		try {
			const { invoke } = await import("@tauri-apps/api/core");
			await invoke("move_path_to_trash", { path: targetPath, projectPath: this.currentProjectPath });
			this.resourceStatus = t("packages.editor.movedToTrash");
			this.commandStatus = this.resourceStatus;
			await this.refreshDiscoveredResources();
		} catch (err) {
			this.resourceEditorError = err instanceof Error ? err.message : String(err);
	...
	private renderResourceEditorModal(): TemplateResult | typeof nothing {
		...
		const saveLabel = this.creatingResource ? t("packages.editor.saving") : creating ? t("packages.editor.create") : t("packages.editor.saveChanges");
		...
		return html`
			<div class="overlay" @click=${(event: Event) => event.target === event.currentTarget && this.closeResourceEditor()}>
				<div class="overlay-card packages-config-modal">
					<div class="overlay-header">
						<div>
							<div class="packages-config-modal-title">${creating ? t("packages.editor.createTitle") : t("packages.editor.editTitle")}</div>
							<div class="packages-config-modal-sub">${t("packages.editor.sub")}</div>
						</div>
					...
					<div class="overlay-body packages-config-modal-body">
						<div class="packages-config-field-grid">
							<div class="packages-config-field">
								<label class="packages-config-field-label">${t("packages.editor.typeLabel")}</label>
								${this.settingsSelect("resource-editor:kind").render({
									ariaLabel: t("packages.editor.typeLabel"),
									triggerClass: "settings-select-trigger-field",
									disabled: typeLocked || this.creatingResource,
									value: this.resourceEditorKind,
									options: [
										{ value: "prompt", label: t("packages.editor.typePrompt") },
										{ value: "skill", label: t("packages.editor.typeSkill") },
									],
									onSelect: (value) => {
						...
						<div class="packages-config-field">
							<label class="packages-config-field-label">${t("packages.editor.nameLabel")}</label>
							<input
								class="packages-config-input"
								type="text"
						...
						<div class="packages-config-field">
							<label class="packages-config-field-label">${t("packages.editor.descriptionLabel")}</label>
							<input
								class="packages-config-input"
								type="text"
						...
						<div class="packages-config-field">
							<label class="packages-config-field-label">${t("packages.editor.contentLabel")}</label>
							<textarea
								class="packages-config-textarea"
								?disabled=${this.creatingResource}
						...
						<div class="packages-config-modal-actions">
							<button class="ghost-btn" ?disabled=${this.creatingResource} @click=${() => this.applyResourceTemplate()}>${t("packages.editor.useTemplate")}</button>
							<button class="ghost-btn" ?disabled=${this.creatingResource} @click=${() => this.closeResourceEditor()}>${t("common.cancel")}</button>
							<button class="ghost-btn" ?disabled=${this.creatingResource} @click=${() => void this.saveResourceEditor()}>
								${saveLabel}
							</button>
						</div>
						${this.resourceEditorPath && !creating
							? html`<div class="packages-section-submeta">${t("packages.editor.pathLabel", { path: this.resourceEditorPath })}</div>`
							: nothing}
						${this.resourceEditorLoaded && !creating
							? html`<div class="packages-section-submeta">${t("packages.editor.reloadHint")}</div>`
							: nothing}
	...
	private statusTone(): "info" | "success" | "error" {
		...
		if (!text) return "info";
		if (text.includes("failed") || text.includes("error") || text.includes("失败") || text.includes("错误")) return "error";
		if (
			text.startsWith("installed") || text.startsWith("removed") || text.startsWith("updated") || text.startsWith("refreshed") ||
			text.startsWith("已安装") || text.startsWith("已移除") || text.startsWith("已更新") || text.startsWith("已刷新")
		) {
			return "success";
		}
		return "info";
	...
	private async runDesktopNotificationSmokeTest(): Promise<void> {
		if (this.runningCommand) return;
		this.runningCommand = true;
		this.commandStatus = t("packages.notify.testing");
		this.render();
		try {
			...
			if (!granted) {
				this.commandStatus = t("packages.notify.permissionDenied");
				return;
			}
			sendNotification({
				title: "Pi Desktop",
				body: t("packages.notify.testBody"),
				autoCancel: true,
				sound: "Ping",
				extra: { source: "packages-notify-smoke" },
			});
			this.commandOutput += `${this.commandOutput ? "\n" : ""}[notify-test] sent via tauri notification plugin\n`;
			this.commandStatus = t("packages.notify.testSent");
		} catch (err) {
			const message = err instanceof Error ? err.message : String(err);
			this.commandOutput += `${this.commandOutput ? "\n" : ""}[notify-test] failed: ${message}\n`;
			this.commandStatus = t("packages.notify.testFailed", { message });
		} finally {
			this.runningCommand = false;
			this.render();
		}
	...
	private buildExtensionSurfaceItems(
		...
		for (const item of discover) {
			...
			bySource.set(normalized, {
				...
				displayName: item.name,
				source,
				description: item.description || t("packages.meta.noDescription"),
				note: `v${item.version}`,
				openUrl: item.npmUrl,
				sourceKind: "npm",
				installState,
	...
	private async openPackagesItemModal(modal: ActivePackagesModal): Promise<void> {
		...
		if (modal.kind === "skill") {
			...
			const fallbackMessage = modal.item.packageSource
				? t("packages.skillDoc.unavailableRefresh")
				: t("packages.skillDoc.unavailableShort");
		...
		const fallbackMessage = modal.item.installed
			? t("packages.skillDoc.unavailableRefresh")
			: t("packages.skillDoc.installToView");
	...
	private async openPath(path: string): Promise<void> {
		...
		try {
			...
			throw new Error(t("packages.status.pathNotFound"));
		} catch (err) {
			const message = err instanceof Error ? err.message : String(err);
			this.commandStatus = t("packages.status.openFolderFailed", { message });
			this.render();
		}
	}
	...
	private async installExtensionItem(item: ExtensionSurfaceItem): Promise<void> {
		if (normalizeRecommendedSource(item.source) === DESKTOP_THEMES_PACKAGE_SOURCE) {
			if (this.runningCommand || this.runningConfigCommand) return;
			this.runningCommand = true;
			this.commandStatus = t("packages.themes.installing");
			this.render();
			try {
				const result = await restoreBundledThemes();
				await this.refreshBundledThemesStatus();
				this.commandStatus = t("packages.themes.installed", { created: result.created, updated: result.updated, renamed: result.renamed });
			} catch (err) {
				this.commandStatus = t("packages.themes.installFailed", { message: err instanceof Error ? err.message : String(err) });
			} finally {
				this.runningCommand = false;
				this.render();
			}
	...
	private async uninstallExtensionItem(item: ExtensionSurfaceItem): Promise<void> {
		...
		if (normalizeRecommendedSource(item.source) === DESKTOP_THEMES_PACKAGE_SOURCE) {
			this.runningCommand = true;
			this.commandStatus = t("packages.themes.uninstalling");
			this.render();
			try {
				const result = await removeBundledThemes();
				await this.refreshBundledThemesStatus();
				this.commandStatus = t("packages.themes.uninstalled", { removed: result.removed, removedLegacy: result.removedLegacy });
			} catch (err) {
				this.commandStatus = t("packages.themes.uninstallFailed", { message: err instanceof Error ? err.message : String(err) });
			} finally {
				this.runningCommand = false;
				this.render();
			}
	...
	private async uninstallThemeItem(item: DiscoveredThemeItem): Promise<void> {
		if (this.runningCommand || this.runningConfigCommand) return;
		this.runningCommand = true;
		this.commandStatus = t("packages.themes.uninstallingTheme", { name: item.name });
		this.render();
		try {
			...
			this.closePackagesItemModal();
			await this.refreshPackages(false);
			this.commandStatus = t("packages.themes.uninstalledTheme", { name: item.name });
		} catch (err) {
			this.commandStatus = t("packages.themes.uninstallThemeFailed", { name: item.name, message: err instanceof Error ? err.message : String(err) });
		} finally {
			this.runningCommand = false;
			this.render();
		}
	...
	private renderSkillContentBlock(): TemplateResult {
		if (this.activeSkillContentLoading) {
			return html`
				<div class="packages-skill-content">
					<div class="ui-loading" role="status" aria-label=${t("packages.skillDoc.loading")}><span class="ui-loading-spinner"></span></div>
				</div>
			`;
		}
				...
				<div class="packages-skill-content">
					<div class="packages-skill-doc">
						...
						${sections.length > 0
							...
							`)
							: html`<div class="packages-empty">${t("packages.skillDoc.empty")}</div>`}
					</div>
				</div>
			`;
		}
		...
		return html`
			<div class="packages-skill-content">
				<div class="packages-empty">${t("packages.skillDoc.unavailable")}</div>
			</div>
		`;
	}
	...
	private renderPackagesItemModal(): TemplateResult | typeof nothing {
		...
		if (modal.kind === "extension") {
			...
			const pathNote = installedForScope?.location || t("packages.actions.notInstalled");
			return html`
					...
					<div class="overlay-card packages-config-modal packages-item-modal">
						...
						<div class="overlay-body packages-config-modal-body">
							...
							${this.renderItemMetaRows([
								{ label: t("packages.meta.source"), value: item.source },
								{ label: t("packages.meta.status"), value: selectedScopeInstalled ? t("packages.actions.installed") : t("packages.actions.notInstalled") },
								{ label: t("packages.meta.path"), value: pathNote },
							])}

							${commands.length > 0
								? html`
									<div class="packages-section-submeta">${t("packages.config.extensionSettings")}</div>
									${commands.map((command) => this.renderPackageConfigCommandEditor(source, command))}
								`
								: html`<div class="packages-empty">${t("packages.config.noSettingsCommands")}</div>`}
							${this.packageConfigStatus ? html`<div class="packages-section-submeta">${this.packageConfigStatus}</div>` : nothing}
							...
							<div class="packages-config-modal-actions">
								<button class="ghost-btn" ?disabled=${this.runningCommand} @click=${() => void (selectedScopeInstalled ? this.uninstallExtensionItem(item) : this.installExtensionItem(item))}>
									${this.runningCommand ? (selectedScopeInstalled ? t("packages.actions.uninstalling") : t("packages.actions.installing")) : selectedScopeInstalled ? t("packages.actions.uninstall") : t("packages.actions.install")}
								</button>
								${item.openUrl ? html`<button class="ghost-btn" @click=${() => void this.openExternal(item.openUrl!)}>${t("packages.actions.openPage")}</button>` : nothing}
								${installedForScope?.location ? html`<button class="ghost-btn" @click=${() => void this.openPath(installedForScope.location)}>${t("packages.actions.openFolder")}</button>` : nothing}
							</div>
						</div>
					</div>
				</div>
				...
				<div class="overlay" @click=${(event: Event) => event.target === event.currentTarget && this.closePackagesItemModal()}>
					<div class="overlay-card packages-config-modal packages-item-modal">
						...
						<div class="overlay-body packages-config-modal-body">
							${item.description ? html`<div class="packages-section-desc">${item.description}</div>` : nothing}
							${this.renderItemMetaRows([
								{ label: t("packages.meta.type"), value: item.variant === "dark" ? t("packages.kinds.themeDark") : t("packages.kinds.themeLight") },
								{ label: t("packages.meta.path"), value: item.path },
							])}
							<div class="packages-theme-preview-grid">
								<div class="packages-theme-preview-row">
									<span>${t("packages.meta.accent")}</span>
								...
								<div class="packages-theme-preview-row">
									<span>${t("packages.meta.background")}</span>
								...
								<div class="packages-theme-preview-row">
									<span>${t("packages.meta.foreground")}</span>
							...
							<div class="packages-config-modal-actions">
								<button class="ghost-btn danger" ?disabled=${this.runningCommand || this.runningConfigCommand} @click=${() => void this.uninstallThemeItem(item)}>
									${this.runningCommand ? t("packages.actions.uninstalling") : t("packages.actions.uninstall")}
								</button>
								<button class="ghost-btn" @click=${() => void this.openPath(pathDirName(item.path))}>${t("packages.actions.openFolder")}</button>
							</div>
						</div>
					</div>
				</div>
		...
		if (modal.kind === "recommended-skill") {
			...
			const primaryLabel = this.runningCommand
				? item.installed
					? t("packages.actions.preparing")
					: t("packages.actions.installing")
				: item.installed
					? t("packages.actions.tryInChat")
					: t("packages.actions.installAndTry");
			return html`
				<div class="overlay" @click=${(event: Event) => event.target === event.currentTarget && this.closePackagesItemModal()}>
					<div class="overlay-card packages-config-modal packages-item-modal">
						<div class="overlay-header">
						...
						<div class="overlay-body packages-config-modal-body">
							...
							${this.renderItemMetaRows([
								{ label: t("packages.meta.command"), value: commandText },
								{ label: t("packages.meta.status"), value: item.installed ? t("packages.actions.installed") : t("packages.actions.notInstalled") },
								{ label: t("packages.meta.path"), value: skillPath ?? t("packages.meta.installToViewLocation") },
							])}
							${skillContent}
							...
							<div class="packages-config-modal-actions">
								...
								${skillPath ? html`<button class="ghost-btn" @click=${() => void this.openPath(pathDirName(skillPath))}>${t("packages.actions.openFolder")}</button>` : nothing}
								${skillOpenUrl ? html`<button class="ghost-btn" @click=${() => void this.openExternal(skillOpenUrl)}>${t("packages.actions.openPage")}</button>` : nothing}
							</div>
						</div>
					</div>
				</div>
			...
			<div class="overlay" @click=${(event: Event) => event.target === event.currentTarget && this.closePackagesItemModal()}>
				<div class="overlay-card packages-config-modal packages-item-modal">
					...
					<div class="overlay-body packages-config-modal-body">
						${item.description ? html`<div class="packages-section-desc">${item.description}</div>` : nothing}
						${this.renderItemMetaRows([
							{ label: t("packages.meta.command"), value: item.commandText },
							{ label: t("packages.meta.source"), value: item.loaded ? t("packages.meta.runtime") : t("packages.meta.file") },
							{ label: t("packages.meta.path"), value: skillPath ?? t("packages.meta.runtimeCommand") },
						])}
						${skillContent}
						<div class="packages-config-modal-actions">
							<button class="ghost-btn" ?disabled=${this.runningCommand || this.runningConfigCommand} @click=${() => void this.runSkill(item)}>${t("packages.actions.tryInChat")}</button>
							${skillPath ? html`<button class="ghost-btn" @click=${() => void this.openPath(pathDirName(skillPath))}>${t("packages.actions.openFolder")}</button>` : nothing}
							${skillOpenUrl
								? html`<button class="ghost-btn" @click=${() => void this.openExternal(skillOpenUrl)}>${t("packages.actions.openPage")}</button>`
								: nothing}
							${canDelete
								? html`<button class="ghost-btn danger" ?disabled=${this.deletingResource} @click=${() => void this.deleteResource(item)}>${this.deletingResource ? t("packages.actions.removing") : t("packages.actions.uninstallSkill")}</button>`
								: nothing}
						</div>
					</div>
				</div>
	...
	render(): void {
		...
		const recommendedBadge = html`<span class="packages-card-scope recommended">${t("packages.sections.recommendedBadge")}</span>`;
		...
		for (const item of installedRecommendedSkills) {
			const note = item.resource?.path || t("packages.meta.packagePrefix", { name: this.getDisplayName(item.definition.packageSource) });
			addInstalledRow(item.definition.name.toLowerCase(), {
				title: item.definition.name,
				description: item.resource?.description || item.definition.description,
				note,
				iconName: "skill",
				actions: html`<button class="packages-row-install installed" title=${t("packages.actions.installed")} @click=${() => void this.openPackagesItemModal({ kind: "recommended-skill", item })}>✓</button>`,
		...
		for (const item of visibleSkillResources) {
			addInstalledRow(item.name.toLowerCase(), {
				title: item.name,
				description: item.description || t("packages.meta.skillFallback"),
				note: item.path || (item.packageDisplayName ? t("packages.meta.packagePrefix", { name: item.packageDisplayName }) : t("packages.meta.runtimeCommand")),
				iconName: "skill",
				actions: html`<button class="packages-row-install installed" title=${t("packages.actions.installed")} @click=${() => void this.openPackagesItemModal({ kind: "skill", item })}>✓</button>`,
		...
		for (const item of themeResources) {
			addInstalledRow(`theme:${item.name.toLowerCase()}`, {
				...
				note: item.path,
				iconName: "theme",
				actions: html`<button class="packages-row-install installed" title=${t("packages.actions.installed")} @click=${() => void this.openPackagesItemModal({ kind: "theme", item })}>✓</button>`,
		...
		for (const item of extensionInstalled) {
			addInstalledRow(item.displayName.toLowerCase(), {
				...
				actions: html`<button class="packages-row-install installed" title=${t("packages.actions.installed")} @click=${() => void this.openPackagesItemModal({ kind: "extension", item })}>✓</button>`,
		...
		for (const item of availableRecommendedSkills) {
			addDiscoverRow(0, item.definition.name.toLowerCase(), {
				...
				note: t("packages.meta.packagePrefix", { name: this.getDisplayName(item.definition.packageSource) }),
				iconName: "skill",
				...
				actions: html`
					<button
						...
						title=${t("packages.actions.install")}
						@click=${() => void this.installRecommendedSkill(item)}
					>
						+
					</button>
		...
		const discoverTitle = hasQuery ? t("packages.sections.resultsTitle") : t("packages.sections.recommendedTitle");
		const discoverSubmeta = hasQuery
			? t("packages.sections.resultsFor", { query: queryLabel })
			: t("packages.sections.recommendedSub");
		const discoverEmptyMessage = hasQuery ? t("packages.sections.noMatches") : t("packages.sections.noRecommendations");
			...
			<div class="packages-view-root">
				<div class="packages-view-header">
					<div class="packages-view-title-wrap">
						<div class="packages-view-title">${t("packages.view.title")}</div>
					</div>
					<div class="packages-view-header-actions">
						<button class="packages-back-btn" ?disabled=${this.runningCommand} @click=${() => void this.refreshPackages(true)}>${t("packages.view.refresh")}</button>
						<button class="packages-back-btn" ?disabled=${this.runningCommand || this.runningConfigCommand} @click=${() => void this.triggerCreatorSkillInChat()}>${t("packages.view.createSkill")}</button>
						<button class="packages-back-btn" @click=${() => void this.openCatalog()}>${t("packages.view.browse")}</button>
						${this.onBack
							? html`<button class="packages-back-btn" @click=${() => this.onBack?.()}>${t("packages.view.back")}</button>`
							: nothing}
					</div>
				</div>
				...
				<div class="packages-view-body minimal">
					...
					${this.configError ? html`<div class="packages-banner error">${t("packages.view.configError", { message: this.configError })}</div>` : nothing}
					${this.resourcesError ? html`<div class="packages-banner error">${t("packages.view.resourceError", { message: this.resourcesError })}</div>` : nothing}
					${this.catalogError ? html`<div class="packages-banner error">${t("packages.view.catalogError", { message: this.catalogError })}</div>` : nothing}

					${loadingPackagesInitial
						? html`
							<div class="packages-loading-state ui-skeleton" role="status" aria-label=${t("packages.view.loading")}>
								<span class="skeleton-bar" style="width:34%"></span>
								<span class="skeleton-bar" style="width:56%"></span>
								<span class="skeleton-bar" style="width:44%"></span>
						...
						: html`
							<section class="packages-section">
								<div class="packages-section-head">
									<div>
										<div class="packages-section-title">${t("packages.sections.installedTitle")}</div>
										<div class="packages-section-submeta">${t("packages.sections.installedSub")}</div>
									</div>
								</div>
								${installedLoading
									? html`<div class="ui-loading" role="status" aria-label=${t("packages.sections.installedLoading")}><span class="ui-loading-spinner"></span></div>`
									: this.resourcesRuntimeUnavailable
										? html`<div class="packages-empty">${t("packages.sections.installedNeedsRuntime")}</div>`
										: installedRows.length === 0
											? html`<div class="packages-empty">${t("packages.sections.installedEmpty")}</div>`
											: html`
							...
							<section class="packages-section packages-section-discover">
								...
								${discoverLoading
									? html`<div class="ui-loading" role="status" aria-label=${t("packages.sections.resultsLoading")}><span class="ui-loading-spinner"></span></div>`
							...
							${hasDiagnostics
								? html`
									<details class="packages-diagnostics" ?open=${diagnosticsOpen}>
										<summary>${t("packages.diagnostics.title")}</summary>
										<div class="packages-diagnostics-actions">
											<button class="ghost-btn" ?disabled=${this.runningCommand} @click=${() => void this.updatePackages()}>${t("packages.diagnostics.updatePackages")}</button>
											<button class="ghost-btn" ?disabled=${this.runningCommand} @click=${() => void this.listPackages()}>${t("packages.diagnostics.listPackages")}</button>
											<button class="ghost-btn" ?disabled=${this.runningCommand || this.loadingResources} @click=${() => void this.refreshDiscoveredResources()}>${t("packages.diagnostics.refreshSkills")}</button>
											<button class="ghost-btn" ?disabled=${this.runningCommand} @click=${() => void this.runDesktopNotificationSmokeTest()}>${t("packages.diagnostics.testNotifications")}</button>
											<button class="ghost-btn" ?disabled=${this.runningCommand} @click=${() => this.render()}>${t("packages.diagnostics.refreshLogs")}</button>
										</div>
										<pre class="tool-output packages-command-log">${this.commandOutput || t("packages.diagnostics.noCommands")}</pre>
										<pre class="tool-output packages-command-log">${notifyDebugLines.length > 0 ? notifyDebugLines.join("\n") : t("packages.diagnostics.noNotifyTrace")}</pre>
									</details>
								`
								: nothing}
						`}
