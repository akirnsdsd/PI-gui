...
const TRUNCATED_MARKER_PATTERN = /\n\n\[PI_DESKTOP_TRUNCATED:(\d+)KB\]/g;

/**
 * Rust 端 get_session_page 截断超大字符串后追加 `[PI_DESKTOP_TRUNCATED:<n>KB]`
 * 占位标记；展示前替换为本地化文案。
 */
export function localizeTruncationMarkers(text: string): string {
	if (!text.includes("[PI_DESKTOP_TRUNCATED:")) return text;
	return text.replace(TRUNCATED_MARKER_PATTERN, (_raw, kb: string) =>
		t("timeline.history.truncatedMarker", { kb }),
	);
}

export function extractTextContent(content: unknown): string {
	return localizeTruncationMarkers(extractTextContentRaw(content));
}

function extractTextContentRaw(content: unknown): string {
	if (!content) return "";
...
export function extractToolOutputText(payload: unknown, depth = 0): string {
	const text = extractToolOutputTextRaw(payload, depth);
	// 递归内部调用走 raw；只有最外层结果需要做标记本地化。
	return depth === 0 ? localizeTruncationMarkers(text) : text;
}

function extractToolOutputTextRaw(payload: unknown, depth = 0): string {
	if (depth > 6 || payload === null || typeof payload === "undefined") return "";
	if (typeof payload === "string") return payload;
	if (typeof payload === "number" || typeof payload === "boolean") return String(payload);
	if (Array.isArray(payload)) {
		const parts = payload
			.map((item) => extractToolOutputTextRaw(item, depth + 1).trim())
			.filter(Boolean);
		return parts.join("\n").trim();
	}
	...
	const textFirst = extractTextContentRaw(source.content ?? payload).trim();
	const chunks: string[] = textFirst ? [textFirst] : [];
	const append = (value: unknown): void => {
		const text = extractToolOutputTextRaw(value, depth + 1).trim();
		if (!text) return;
