...
export function summarizeToolCall(
	...
	if (name === "bash" && command) return t("timeline.tools.ranCommand", { command: truncateText(command, 84) });
	if ((name === "read" || name === "readfile") && path) return t("timeline.tools.read", { path: truncateText(path, 74) });
	if ((name === "write" || name === "writefile") && path) return t("timeline.tools.wrote", { path: truncateText(path, 74) });
	if (name === "edit" && path) return t("timeline.tools.edited", { path: truncateText(path, 74) });
	if (name.includes("search") && query) return t("timeline.tools.searched", { query: truncateText(query, 74) });
	if ((name === "list" || name.includes("ls")) && path) return t("timeline.tools.listed", { path: truncateText(path, 74) });
	if (path) return `${toolCall.name} ${truncateText(path, 74)}`;
	return t("timeline.tools.ranTool", { name: toolCall.name });
}
