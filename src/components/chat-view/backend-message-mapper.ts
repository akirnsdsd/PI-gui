...
import { localizeTruncationMarkers } from "./message-content-utils.js";
			...
			case "assistant": {
				...
				for (const part of content) {
					...
					if (type === "text" && typeof p.text === "string") {
						text += localizeTruncationMarkers(p.text);
					}
					const typeLower = (type ?? "").toLowerCase();
					if (typeLower === "thinking" || typeLower === "reasoning" || typeLower.includes("thinking") || typeLower.includes("reason")) {
						if (typeof p.thinking === "string") thinking += localizeTruncationMarkers(p.thinking);
						else if (typeof p.reasoning === "string") thinking += localizeTruncationMarkers(p.reasoning);
						else if (typeof p.text === "string") thinking += localizeTruncationMarkers(p.text);
					}
					if (type === "toolCall") {
						...
						const toolCall: ToolCallBlockLike = {
							id,
							name: typeof p.name === "string" ? p.name : t("timeline.tools.fallbackName"),
				...
				if (toolCallId && toolCallMap.has(toolCallId)) {
					...
					if (target) {
						target.toolCalls.push({
							...
							name: (typeof raw.toolName === "string" && raw.toolName.trim().length > 0
								? raw.toolName
								: t("timeline.tools.fallbackName")) as string,
							args: {},
							result: content || t("timeline.tools.noOutput"),
							isError,
							isRunning: false,
							isExpanded: false,
							startedAt: Date.now(),
						...
						mapped.push({
							...
							sessionEntryId,
							role: "system",
							text: `${isError ? t("timeline.tools.resultHeadingError") : t("timeline.tools.resultHeading")}\n${content || t("timeline.tools.noOutput")}`,
							label: t("timeline.labels.toolResult"),
							toolCalls: [],
						});
					}
				}
