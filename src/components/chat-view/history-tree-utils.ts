...
function mapSessionTreeEntry(
	...
	switch (type) {
		case "message": {
			...
			if (messageRole === "user") {
				role = "user";
				entryLabel = t("chatMisc.history.roles.user");
				preview = extractText(messageRecord?.content).replace(/\s+/g, " ").trim() || t("chatMisc.history.tree.emptyMessage");
			...
			if (messageRole === "assistant") {
				role = "assistant";
				entryLabel = t("chatMisc.history.roles.assistant");
				...
				preview = contentPreview || (stopReason === "aborted" ? t("chatMisc.history.tree.aborted") : errorMessage || t("chatMisc.history.tree.noContent"));
				displayText = `${entryLabel}: ${preview}`;
				break;
			}
			...
			if (messageRole === "toolresult") {
				role = "system";
				entryLabel = t("chatMisc.history.tree.tool");
				const toolName = pickString(messageRecord ?? {}, ["toolName", "tool_name"]) ?? t("chatMisc.history.tree.tool");
				const toolOutputRaw = extractToolOutput(messageRecord?.content ?? messageRecord?.result ?? messageRecord ?? {});
				preview = toolOutputRaw.replace(/\s+/g, " ").trim() || t("chatMisc.history.tree.noOutput");
			...
			entryLabel = messageRoleRaw || t("chatMisc.history.tree.message");
		...
		case "custom_message": {
			role = "custom";
			const customType = pickString(record, ["customType", "custom_type"]) ?? t("chatMisc.history.tree.custom");
			entryLabel = customType;
			preview = extractText(record.content).replace(/\s+/g, " ").trim() || t("chatMisc.history.tree.empty");
			displayText = `[${customType}]: ${preview}`;
			break;
		}
		case "branch_summary": {
			role = "system";
			entryLabel = t("chatMisc.history.tree.branchSummary");
			preview = (pickString(record, ["summary"]) ?? extractText(record.content)).replace(/\s+/g, " ").trim() || t("chatMisc.history.tree.empty");
			displayText = `[${entryLabel}]: ${truncateText(preview, 180)}`;
			break;
		}
		case "compaction": {
			role = "system";
			entryLabel = t("chatMisc.history.tree.compaction");
			const tokensBefore = pickNumber(record, ["tokensBefore", "tokens_before"]);
			preview = (pickString(record, ["summary"]) ?? t("chatMisc.history.tree.compactionEntry")).replace(/\s+/g, " ").trim();
			const tokensBadge = typeof tokensBefore === "number" && Number.isFinite(tokensBefore)
				...
				: t("chatMisc.history.tree.summary");
			displayText = `[${entryLabel}: ${tokensBadge}] ${truncateText(preview, 160)}`;
			break;
		}
		case "thinking_level_change": {
			role = "system";
			entryLabel = t("chatMisc.history.tree.thinking");
			const level = pickString(record, ["thinkingLevel", "thinking_level"]) ?? t("chatMisc.history.tree.thinkingUpdated");
			preview = level;
			displayText = `[${entryLabel}: ${level}]`;
			break;
		}
		case "model_change": {
			role = "system";
			entryLabel = t("chatMisc.history.tree.model");
		...
		case "session_info": {
			role = "system";
			entryLabel = t("chatMisc.history.tree.title");
			preview = pickString(record, ["name"]) ?? t("chatMisc.history.tree.untitled");
			displayText = `[${entryLabel}: ${preview}]`;
			break;
		}
		case "custom": {
			...
			const customType = pickString(record, ["customType", "custom_type"]) ?? t("chatMisc.history.tree.custom");
			entryLabel = customType;
			preview = extractText(record.data).replace(/\s+/g, " ").trim() || t("chatMisc.history.tree.customEntry");
			displayText = `[${t("chatMisc.history.tree.custom")}: ${customType}] ${truncateText(preview, 140)}`;
			break;
		}
		default: {
			role = "system";
	...
	return {
		...
		displayText: normalizedDisplayText || `${entryLabel}: ${normalizedPreview || t("chatMisc.history.tree.empty")}`,
		canFork,
	};
}
