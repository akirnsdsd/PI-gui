...
export function renderAssistantWorkflowView({
	...
	if (completed > 0) summaryParts.push(t("timeline.workflow.summary.complete", { count: completed }));
	if (failed > 0) summaryParts.push(t("timeline.workflow.summary.failed", { count: failed }));
	if (running > 0) summaryParts.push(t("timeline.workflow.summary.running", { count: running }));
	if (summaryParts.length === 0 && total > 0) summaryParts.push(t("timeline.workflow.summary.complete", { count: total }));
				...
				<div class="assistant-block">
					...
					${expanded
						? html`
							<div class="tool-workflow-list">
								${detailEntries.map((entry) => {
									...
									const statusLabel = groupRunning
										? t("timeline.workflow.status.running")
										: groupFailed
											? t("timeline.workflow.status.failed")
											: t("timeline.workflow.status.success");
									return html`
											...
											${groupExpanded
												? html`
													<div class="tool-workflow-details">
														<pre class="tool-workflow-output">${output || t("timeline.workflow.noOutput")}${groupRunning ? html`<span class="streaming-inline"></span>` : nothing}</pre>
							...
							${hasFinalContent ? html`<div class="assistant-final-divider"><span>${t("timeline.workflow.agent")}</span></div>` : nothing}
							${workflow.finalText
