...
export function renderSlashPaletteView({
	...
	if (loading && items.length === 0) {
		return html`<div class="composer-slash-menu"><div class="composer-slash-empty ui-loading-host" role="status" aria-label=${t("composer.slashPalette.loading")}><span class="ui-loading-spinner small"></span></div></div>`;
	}
	if (items.length === 0) {
		return html`<div class="composer-slash-menu"><div class="composer-slash-empty">${t("composer.slashPalette.noMatch", { query })}</div></div>`;
	}
