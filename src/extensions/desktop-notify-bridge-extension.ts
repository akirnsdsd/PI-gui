 ...
 *
 * This bridge emits one notify on agent_settled (pi ≥0.81 extension API event;
 * fires only when the run is fully done — after retries, compaction retries and
 * queued follow-ups — unlike agent_end):
 * - success => info
...
\t\tif (runHadError) {
\t\t\tnotify(ctx, "任务运行出错，已结束。", "error");
\t\t} else {
\t\t\tnotify(ctx, "当前任务已完成。", "info");
\t\t}
\t\tresetRunState();
\t});
}
	...
	try {
		...
		if (hasExisting && normalizedExisting.length > 0 && !existingContent.includes(DESKTOP_NOTIFY_BRIDGE_MARKER)) {
			return {
				...
				updated: false,
				skipped: true,
				error: `已跳过写入通知桥接扩展：${DESKTOP_NOTIFY_BRIDGE_EXTENSION_FILE} 由用户自行管理。`,
			};
		}
