...
/**
 * Bump when bundled theme *values* change (not the schema). The installer
 * rewrites bundled files whose contentVersion is missing or older.
 */
export const PI_DESKTOP_THEME_CONTENT_VERSION = 2;

export function readPiThemeContentVersion(input: unknown): number {
	if (!input || typeof input !== "object" || Array.isArray(input)) return 0;
	const piDesktop = (input as Record<string, unknown>).piDesktop;
	if (!piDesktop || typeof piDesktop !== "object" || Array.isArray(piDesktop)) return 0;
	const version = (piDesktop as Record<string, unknown>).contentVersion;
...
export function buildPiThemeDocument(options: BuildPiThemeDocumentOptions): PiThemeDocument {
	...
	return {
		...
		piDesktop: {
			...
			contentVersion: PI_DESKTOP_THEME_CONTENT_VERSION,
