...
function buildPiThemeOverlay(
	...
	// Codex-style neutral derivation; each mix sums to 100%.
	if (background) {
		const mixes = mode === "dark"
		...
		result["--color-bg-elevated"] = `color-mix(in srgb, ${background} ${mixes.elevated})`;
		result["--color-bg-muted"] = `color-mix(in srgb, ${background} ${mixes.muted})`;
		result["--color-bg-soft"] = `color-mix(in srgb, ${background} ${mixes.soft})`;
		result["--color-bg-sidebar"] = `color-mix(in srgb, ${background} ${mixes.sidebar})`;
		result["--color-bg-workspace-chrome"] = `color-mix(in srgb, ${background} ${mixes.chrome})`;
		result["--color-bg-workspace-chrome-soft"] = `color-mix(in srgb, ${background} ${mixes.chromeSoft})`;
	}

	if (foreground) {
		const secondaryMix = mode === "dark" ? 60 : 68;
		const tertiaryMix = mode === "dark" ? 39 : 33;
		result["--color-text-primary"] = foreground;
		if (background) {
			result["--color-text-secondary"] = `color-mix(in srgb, ${foreground} ${secondaryMix}%, ${background} ${100 - secondaryMix}%)`;
			result["--color-text-tertiary"] = `color-mix(in srgb, ${foreground} ${tertiaryMix}%, ${background} ${100 - tertiaryMix}%)`;
		} else {
			result["--color-text-secondary"] = `color-mix(in srgb, ${foreground} ${secondaryMix}%, transparent)`;
			result["--color-text-tertiary"] = `color-mix(in srgb, ${foreground} ${tertiaryMix}%, transparent)`;
		}
		// Keep border subtle and neutral-ish relative to foreground.
		result["--color-border-default"] = `color-mix(in srgb, ${foreground} ${mode === "dark" ? 10.5 : 11}%, transparent)`;
	}
