import { buildPiThemeDocument, isThemeDocumentSchemaCompatible, PI_DESKTOP_THEME_CONTENT_VERSION, readPiThemeContentVersion } from "./pi-theme-document.js";
...
async function hasBundledThemeRepairCandidate(
	...
	for (const spec of BUNDLED_THEME_SPECS) {
		...
		if (await exists(targetPath)) {
			...
			if (readPiThemeContentVersion(doc) < PI_DESKTOP_THEME_CONTENT_VERSION) {
				return true;
			}
		}
	}
...
async function installBundledThemes(options: { respectMarker: boolean }): Promise<BundledThemeInstallResult> {
	...
	for (const spec of BUNDLED_THEME_SPECS) {
		...
		if (!(await exists(targetPath))) {
			...
			if (!isThemeDocumentSchemaCompatible(existingDoc) || readPiThemeContentVersion(existingDoc) < PI_DESKTOP_THEME_CONTENT_VERSION) {
				const doc = toPiThemeDocument(spec);
				await writeTextFile(targetPath, `${JSON.stringify(doc, null, 2)}\n`);
				updated += 1;
			}
